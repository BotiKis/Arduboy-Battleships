...
ArdBitmap::ArdBitmap(uint8_t *screenBuffer, int16_t width, int16_t height){
...
void ArdBitmap::drawCompressed(int16_t sx, int16_t sy, const uint8_t *compBitmap, uint8_t color, uint8_t align, uint8_t mirror)
{
  ...
  if (sx + w < 0 || sx > sWidth - 1 || sy + h < 0 || sy > sHeight - 1)
    return;
  ...
  while (a < rows && a > -1) {
    ...
    for (uint16_t i = 0; i < len; i++)
    {
      ...
      if (characterPos == 0xFF){
        ...
        if (decByte && bRow < (sHeight / 8) && iColMod + sx < sWidth && iColMod + sx >= 0){

          bitmap_data = decByte * mul_amt;

          if (bRow >= 0) {

            data = sBuffer[(bRow * sWidth) + sx + iColMod];
            if (color) {
              data |= bitmap_data & 0xFF;
            }else {
              data &= ~(bitmap_data & 0xFF);
            }
            sBuffer[(bRow * sWidth) + sx + iColMod] = data;
          }

          if (yOffset && bRow < (sHeight / 8) - 1 && bRow > -2) {

            data = sBuffer[((bRow + 1) * sWidth) + sx + iColMod];
            if (color) {
              data |= ((bitmap_data >> 8) & 0xFF);
            } else {
              data &= ~(((bitmap_data >> 8) & 0xFF));
            }
            sBuffer[((bRow + 1)*sWidth) + sx + iColMod] = data;
          }
        }
...
void ArdBitmap::drawCompressedResized(int16_t sx, int16_t sy, const uint8_t *compBitmap, uint8_t color,uint8_t align, uint8_t mirror, float resize)
{
  ...
  // No need to draw at all if we're offscreen
  if (sx + wRes < 0 || sx > sWidth - 1 || sy + hRes < 0 || sy > sHeight - 1)
    return;
  ...
  while (a < rows && /*a > -1*/ a != 0xFF) {
    ...
    for (uint16_t i = 0; i < len; i++)
    {
      ...
      if (characterPos == 0xFF){
        ...
        if (x_LUT[iCol] != 0xFF && iColMod < sWidth && iColMod >= 0){
          ...
          for (uint8_t s = 0; s < 8 ;s++){
            ...
            if (y_LUT[aRow8+s] != 0xFF && decByte &  BIT_SHIFT[s]){
              ...
              if (row < (sHeight / 8)) {
                ...
                if (color) {
                  sBuffer[(row*sWidth) + (uint8_t)iColMod] |=   BIT_SHIFT[((uint8_t)(y_LUT[aRow8+s]+sy) % 8)];
                } else {
                  sBuffer[(row*sWidth) + (uint8_t)iColMod] &= ~ BIT_SHIFT[((uint8_t)(y_LUT[aRow8+s]+sy) % 8)];
                }
              }
...
void ArdBitmap::drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap, uint8_t w, uint8_t h, uint8_t color, uint8_t align, uint8_t mirror)
{
  ...
  if (x + w <= 0 || x > sWidth - 1 || y + h <= 0 || y > sHeight - 1)
    return;
  ...
  if (x + w > sWidth - 1) {
    rendered_width = ((sWidth - x) - xOffset);
  } else {
  ...
  // if (sRow + loop_h - 1 > (sHeight/8)-1)
  if (sRow + loop_h > (sHeight / 8)) {
    loop_h = (sHeight / 8) - sRow;
  }
      ...
      for (uint8_t a = 0; a < loop_h; a++) {
        for (uint8_t iCol = 0; iCol < rendered_width; iCol++) {
          ...
          if(data) {
            ...
            if (yOffset != 0 && sRow < 7) {
              data = sBuffer[ofs + sWidth];
              if (color){
                data |= (bitmap_data >> 8) & 0xFF;
              } else{
                data &= ~((bitmap_data >> 8) & 0xFF);
              }
              sBuffer[ofs + sWidth] = data;
            }
          }
          ofs++;
